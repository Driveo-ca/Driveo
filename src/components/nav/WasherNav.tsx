...
export function WasherNav() {
  ...
  return (
    <>
      {/* Desktop top nav */}
      <nav className="hidden md:flex items-center justify-between px-8 py-4 border-b border-border bg-card sticky top-0 z-50">
        <Link href="/washer/dashboard" className="flex items-center gap-2.5 group">
          ...
          <span className="font-display text-xl text-foreground tracking-wide">DRIVEO</span>
              ...
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ...
                    : 'text-foreground/60 dark:text-foreground/55 hover:text-foreground hover:bg-foreground/[0.07] dark:hover:bg-foreground/[0.04]'
                )}
              >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50">
        <div className="bg-card border-t border-border px-2 py-2 flex justify-around">
          {navItems.map((item) => {
            const isActive = pathname.startsWith(item.href);
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive ? 'text-[#E23232]' : 'text-foreground/60 dark:text-foreground/55 active:scale-95'
                )}
              >
