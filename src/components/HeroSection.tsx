      ...
      <div className="relative z-10 max-w-[1600px] mx-auto w-full flex flex-col gap-10">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="w-full max-w-xl border border-white/20 p-5 sm:p-8 rounded-3xl backdrop-blur-md bg-black/40" onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
          ...
          <div className="flex justify-between items-center">
            ...
            <Link href="/auth/signup" className="bg-[#E23232] text-white font-mono text-xs uppercase tracking-widest px-8 py-4 rounded-full hover:bg-white hover:text-black transition-all" onClick={() => trackCtaClick('hero')} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
              Book Now
            </Link>
          </div>
        </motion.div>
