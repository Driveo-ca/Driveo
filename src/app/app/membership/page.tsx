...
function MembershipPage() {
  ...
  const [allSubscriptions, setAllSubscriptions] = useState<Array<{ subscription: SubscriptionData; usage: UsageData | null }>>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      if (usageRes.subscriptions) setAllSubscriptions(usageRes.subscriptions);
      setLoading(false);
    }
    load();
      ...
      <div className="min-h-screen text-foreground">
        <div className="max-w-xl mx-auto px-5 md:px-10 py-8">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-foreground/55 hover:text-foreground text-sm transition-colors mb-6"
          >
            <X className="w-4 h-4" />
            Cancel
          </button>
          ...
          <div className="flex items-center gap-3 mb-6 p-4 rounded-xl border border-border/40 bg-foreground/[0.02]">
            <div className={cn(
              ...
              <Icon className={cn('w-[18px] h-[18px]', meta.featured ? 'text-[#E23232]' : 'text-foreground/55')} />
            </div>
            <div className="flex-1">
              <p className="font-semibold text-foreground text-sm">{checkoutPlan.name}</p>
              <p className="text-[11px] text-foreground/55">{checkoutPlan.washes_per_month} washes/month</p>
            </div>
  ...
  if (allSubscriptions.length > 0) {
    const subscribedPlanIds = new Set(allSubscriptions.map(s => s.subscription.planId));
    const availablePlans = plans.filter(p => !subscribedPlanIds.has(p.id));
          ...
          {justSubscribed && (
            <div className="mb-6 p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.06] flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-foreground/55 mt-0.5">Your subscription is now active. Book your first wash!</p>
              </div>
            </div>
          )}
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-foreground tracking-tight">Your Membership</h1>
            <p className="text-xs text-foreground/55 mt-1">Manage your Driveo Plus subscription{allSubscriptions.length > 1 ? 's' : ''}</p>
          </div>

          {/* All active subscriptions */}
          <div className="space-y-4 mb-6">
            {allSubscriptions.map(({ subscription: sub, usage: subUsage }) => {
              const meta = PLAN_META[sub.plan.wash_plan] || PLAN_META.regular;
              const SIcon = meta.icon;
              const usagePercent = subUsage ? Math.min((subUsage.used / subUsage.allocated) * 100, 100) : 0;
              const remaining = subUsage ? subUsage.allocated - subUsage.used : 0;
              const isCancelling = cancelling === sub.id;

              return (
                <div key={sub.id} className="relative rounded-2xl border border-[#E23232]/20 overflow-hidden">
                  ...
                  <div className="relative p-6">
                    <div className="flex items-start justify-between mb-6">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-[#E23232]/10 flex items-center justify-center">
                          <SIcon className="w-5 h-5 text-[#E23232]" />
                        </div>
                        <div>
                          <p className="font-bold text-foreground text-lg">{sub.plan.name}</p>
                          <div className="flex items-baseline gap-1">
                            <span className="text-2xl font-bold text-foreground">${(sub.plan.monthly_price / 100).toFixed(0)}</span>
                      ...
                      <span className={cn(
                        ...
                        <span className={cn('w-1.5 h-1.5 rounded-full', sub.cancelAtPeriodEnd ? 'bg-amber-500' : 'bg-emerald-500')} />
                    ...
                    {subUsage && (
                      <div className="rounded-xl bg-foreground/[0.03] border border-border/40 p-4 mb-4">
                        <div className="flex items-center justify-between text-sm mb-3">
                          ...
                          <span className="text-foreground font-bold tabular-nums">{subUsage.used} / {subUsage.allocated}</span>
                        </div>
                        ...
                        <p className="text-[11px] text-foreground/55 mt-2">{remaining} wash{remaining !== 1 ? 'es' : ''} remaining</p>
                      </div>
                    )}
                    ...
                    {sub.currentPeriodEnd && (
                      <p className="text-xs text-foreground/55">
                        ...
                        {new Date(sub.currentPeriodEnd).toLocaleDateString('en-CA', { month: 'long', day: 'numeric', year: 'numeric' })}
                      </p>
                    )}
                    ...
                    {!sub.cancelAtPeriodEnd && (
                      <button
                        ...
                        className="w-full mt-5 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border/50 text-foreground/55 hover:text-red-500 hover:border-red-500/20 hover:bg-red-500/[0.04] text-sm font-medium transition-all disabled:opacity-50"
                      >
                        {isCancelling ? <Loader2 className="w-4 h-4 animate-spin" /> : <X className="w-3.5 h-3.5" />}
          ...
          {availablePlans.length > 0 && (
            <div>
              <div className="flex items-center gap-3 mb-3">
                <span className="text-[11px] font-semibold uppercase tracking-[0.1em] text-foreground/55">Add another plan</span>
                <div className="h-px flex-1 bg-border/40" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {availablePlans.map(plan => {
                  const uMeta = PLAN_META[plan.wash_plan] || PLAN_META.regular;
                  const UIcon = uMeta.icon;
                  const isSubscribing = subscribing === plan.id;
                  return (
                    <div key={plan.id} className="rounded-xl border border-border/50 hover:border-[#E23232]/30 p-4 transition-all">
                      <div className="flex items-center gap-3 mb-3">
                        <div className="w-9 h-9 rounded-lg bg-[#E23232]/[0.08] flex items-center justify-center">
                          <UIcon className="w-4 h-4 text-[#E23232]" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-foreground">{plan.name}</p>
                          <p className="text-[11px] text-foreground/55">{uMeta.tagline}</p>
                        </div>
                      </div>
                      <div className="flex items-end justify-between">
                        <div>
                          <span className="text-xl font-bold text-foreground">${(plan.monthly_price / 100).toFixed(0)}</span>
                          <span className="text-xs text-foreground/55">/mo</span>
                          <p className="text-[10px] text-foreground/50 mt-0.5">{plan.washes_per_month} washes/month</p>
                        </div>
                        <button
                          onClick={() => handleSubscribe(plan.id)}
                          disabled={subscribing !== null}
                          className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-[#E23232] hover:bg-[#c92a2a] text-white text-xs font-semibold transition-all active:scale-95 disabled:opacity-40"
                        >
                          {isSubscribing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <>Add <ArrowRight className="w-3 h-3" /></>}
                        </button>
                      </div>
                    </div>
                  );
      ...
      <div className="max-w-5xl mx-auto px-5 md:px-10 py-8">
        ...
        {vehicles.length > 0 && (
          <div className="max-w-sm mx-auto mb-8">
            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-foreground/55 mb-2 text-center">Subscription vehicle</p>
                ...
                {selectedVehicle ? (
                  <>
                    ...
                    <div className="flex-1 text-left min-w-0">
                      ...
                      <p className="text-[11px] text-foreground/55 capitalize">{selectedVehicle.type.replace('_', ' ')}</p>
                    </div>
                  </>
                ) : (
                  <span className="text-sm text-foreground/55">Select a vehicle</span>
                )}
                <ChevronDown className={cn('w-4 h-4 text-foreground/50 shrink-0 transition-transform', showVehiclePicker && 'rotate-180')} />
              </button>
                  ...
                  {vehicles.map(v => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-foreground/50 capitalize">{v.type.replace('_', ' ')}{v.is_primary ? ' · Default' : ''}</p>
                      </div>
        ...
        {vehicles.length === 0 && (
          <div className="max-w-sm mx-auto mb-8 p-4 rounded-xl border border-amber-500/20 bg-amber-500/[0.04] text-center">
            ...
            <p className="text-xs text-foreground/55 mt-1">You need at least one vehicle to subscribe</p>
          </div>
        )}
                ...
                <div className={cn('relative flex flex-col flex-1 p-5 md:p-6', meta.featured && 'pt-6')}>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-foreground/55">{meta.tagline}</p>
                    </div>
                  </div>
                  ...
                  <div className="mb-5">
                    ...
                    <p className="text-[11px] text-foreground/50 mt-1">${pricePerWash}/wash · {plan.washes_per_month} washes</p>
                  </div>
                  ...
                  <ul className="space-y-2.5 mb-6 flex-1">
                    {meta.features.map((f) => (
                      <li key={f} className="flex items-center gap-2.5 text-[13px] text-foreground/55">
                        <Check className={cn('w-3.5 h-3.5 shrink-0', meta.featured ? 'text-[#E23232]' : 'text-foreground/50')} strokeWidth={2.5} />
                        {f}
                      </li>
                    ))}
        ...
        {/* Trust bar */}
        <div className="flex items-center justify-center gap-6 mt-10 text-foreground/50">
