    ...
    <div className="max-w-lg mx-auto pb-8">
      ...
      <div className="sticky top-0 z-30 bg-background border-b border-border px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-foreground/55 dark:text-foreground/50 text-[10px]">#{booking.id.slice(0, 8)}</p>
        </div>
        <Badge
          className={cn(
      ...
      <div className="px-4 pt-5 pb-4">
        <div className={cn('flex items-center gap-3 p-4 rounded-2xl border', config.bgColor, 'border-border')}>
          ...
          <div className="flex-1">
            ...
            <p className="text-foreground/60 dark:text-foreground/40 text-xs mt-0.5">{config.description}</p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-surface border border-border rounded-2xl p-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <span className="text-foreground/50 dark:text-foreground/45">·</span>
                  <div className="flex items-center gap-1">
                    <Droplets className="w-3 h-3 text-foreground/55 dark:text-foreground/50" />
            ...
            <div className="mt-4">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[#E23232]/10 border border-[#E23232]/25 hover:bg-[#E23232]/15 transition-colors"
              >
                <MessageCircle className="w-4 h-4 text-[#E23232]" />
                <span className="text-[#E23232] text-sm font-medium">Chat with Washer</span>
              </button>
            </div>
            ...
            {washer.washer_profiles?.vehicle_make && (
              <div className="mt-3 pt-3 border-t border-border flex items-center gap-2">
                ...
                <span className="text-foreground/55 dark:text-foreground/35 text-xs">
      ...
      <div className="px-4 pb-4">
        <button
          ...
          className="w-full flex items-center justify-between py-3 text-foreground/60 dark:text-foreground/40 hover:text-foreground/60 transition-colors"
        >
                ...
                <div key={s} className="flex items-start gap-3">
                  ...
                  <div className="flex flex-col items-center">
                    <div className={cn(
                      ...
                      'bg-foreground/[0.06] dark:bg-foreground/[0.03]'
                    )}>
                      {isPast ? (
                        ...
                        <Circle className="w-4 h-4 text-foreground/30 dark:text-foreground/10" />
                      )}
                    </div>
                  ...
                  <div className="flex-1 flex items-start justify-between pt-1.5 pb-4">
                    <div>
                      ...
                      {isCurrent && (
                        <p className="text-foreground/55 dark:text-foreground/50 text-[11px] mt-0.5">{stepConfig.description}</p>
                      )}
                    </div>
                    {(isPast || isCurrent) && timestamp && (
                      <span className="text-foreground/50 dark:text-foreground/45 text-[10px] pt-0.5">
        ...
        <div className="px-4 pb-4">
          <div className="bg-surface border border-border rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <Camera className="w-4 h-4 text-foreground/55 dark:text-foreground/50" />
              <span className="text-foreground/60 text-xs font-medium uppercase tracking-wider">Before & After</span>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="aspect-[4/3] rounded-xl bg-foreground/[0.06] dark:bg-foreground/[0.03] border border-border flex flex-col items-center justify-center gap-2">
                <Camera className="w-5 h-5 text-foreground/50 dark:text-foreground/45" />
                <span className="text-foreground/50 dark:text-foreground/45 text-xs">Before</span>
              </div>
              <div className="aspect-[4/3] rounded-xl bg-foreground/[0.06] dark:bg-foreground/[0.03] border border-border flex flex-col items-center justify-center gap-2">
                <Sparkles className="w-5 h-5 text-foreground/50 dark:text-foreground/45" />
                <span className="text-foreground/50 dark:text-foreground/45 text-xs">After</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="px-4 pb-4">
        ...
        {showDetails && (
          <div className="bg-surface border border-border rounded-2xl p-4 space-y-3 text-sm">
            ...
            <div className="flex items-center gap-3">
              <MapPin className="w-4 h-4 text-foreground/55 dark:text-foreground/50" />
              <span className="text-foreground/70">{booking.service_address}</span>
            </div>
            {booking.location_notes && (
              <div className="flex items-start gap-3">
                <MessageCircle className="w-4 h-4 text-foreground/55 dark:text-foreground/50 mt-0.5" />
                <span className="text-foreground/50 text-xs">{booking.location_notes}</span>
              </div>
            )}
            <div className="flex items-center gap-3">
              <Clock className="w-4 h-4 text-foreground/55 dark:text-foreground/50" />
            ...
            <div className="border-t border-border pt-3 space-y-2">
              <div className="flex justify-between">
                <span className="text-foreground/60 dark:text-foreground/40">{PLAN_LABELS[booking.wash_plan]}</span>
                <span className="text-foreground/70">{centsToDisplay(booking.base_price)}</span>
              </div>
              {booking.vehicle_multiplier !== 1 && (
                <div className="flex justify-between">
                  <span className="text-foreground/60 dark:text-foreground/40">Vehicle ({booking.vehicle_multiplier}x)</span>
              ...
              {booking.dirt_multiplier !== 1 && (
                <div className="flex justify-between">
                  <span className="text-foreground/60 dark:text-foreground/40">Dirt level {booking.dirt_level} ({booking.dirt_multiplier}x)</span>
              ...
              <div className="flex justify-between">
                <span className="text-foreground/60 dark:text-foreground/40">HST (13%)</span>
              ...
              <p className="text-foreground/50 dark:text-foreground/45 text-[10px] flex items-center gap-1">
      ...
      {status === 'pending' && (
        <div className="px-4">
          <Button
            ...
            className="w-full border-border text-foreground/60 dark:text-foreground/40 hover:text-red-400 hover:border-red-500/20 rounded-xl"
          >
