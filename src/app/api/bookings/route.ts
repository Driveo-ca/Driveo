...
// createNotification is unused now — broadcast is a separate endpoint
...
export async function POST(request: Request) {
  try {
    ...
    await adminSupabase.from('notifications').insert({
      ...
      body: `Your ${price.planLabel} wash has been confirmed. Complete payment to find a washer.`,
      data: { booking_id: booking.id },
    });

    // NOTE: Do NOT broadcast here. Washers are notified only AFTER
    // the customer's card is successfully authorized via Stripe.
    // The frontend calls POST /api/bookings/broadcast after confirmPayment().
